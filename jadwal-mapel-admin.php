									...
									while ($row=mysql_fetch_array($exe)) { $no++;
										...
										<div id="modal<?php echo $id_jadwal ;?>" class="uk-modal">
											<div class="uk-modal-dialog">
												<button type="button" class="uk-modal-close uk-close"></button>
												<div class="uk-modal-header">
													<h2>Lihat Data Jadwal Mata Pelajaran</h2>
												</div>
												...
												<div class="form-group">
													...
													<input type="text" name="guru" required readonly="readonly" disabled id="guruwk" onkeyup="asdedit()" value="<?php echo ucwords($row["nm_guru"]).' {' .$row["nip"]. ' }' ?>" class="form-control" placeholder="Cari Nama / NIP" />  
													<input type="hidden" name="id_gurus" id="id_gurus"  class="form-control" />  
													
													
													<div id="guruListwk"></div>  
												</div>

												<div class="form-group">
													<label>Mata Pelajaran</label>
													<select name="kd_mapel" required readonly="readonly" disabled id="kd_mapelwk" onchange="jikukkelasedit()" class="form-control">
														<option value="">--- Pilih Mapel --</option>
														<?php
												...
												<div class="item form-group">
													<label>Pilih Kelas<span class="required">*</span></label>
													<div>
														<select name="id_kelas" id="id_kelaswk" required readonly disabled  class="form-control">
															<option value=""> Pilih Kelas </option>
															<?php
															
															$kelass =mysql_query("SELECT id_kelas, nm_kelas FROM kelas where id_kelas in (SELECT kelas.id_kelas FROM kelas left join mengajar on mengajar.id_kelas=kelas.id_kelas 
																where mengajar.id_kelas='$row[id_kelas]' and mengajar.id_guru='$row[id_guru]'
																) order by nm_kelas asc");
															while ($datakelass=mysql_fetch_array($kelass)) {
																if ($datakelass['id_kelas']==$row['id_kelas']) {
																	$cek ="selected";
																}
																else{
																	$cek= "";
																}
																echo "<option value=\"$datakelass[id_kelas]\" $cek>$datakelass[nm_kelas]</option>\n";
															}
															?>
												...
												<div class="item form-group">
													<label>Pilih Hari<span class="required">*</span></label>
													<div>
														<select name="id_hari" id="id_hariwk" required readonly disabled class="form-control">
															<option value="">--- Pilih Hari <?php echo $row['id_hari']; ?> --</option>
															<?php
															$queryhari = "SELECT * from hari";
															$hasilhari = mysql_query($queryhari);
															while ($dataharis = mysql_fetch_array($hasilhari)) {
																if ($dataharis['id_hari']==$row['id_hari']) {
																	$cek ="selected";
																}
																else{
																	$cek= "";
																}
																echo "<option value=\"$dataharis[id_hari]\" $cek>$dataharis[nm_hari]</option>\n";
															}
															?>
														</select>
													</div>
												...
												<div class="item form-group">
													<label>Pilih Jam Pelajaran<span class="required">*</span></label>
													<div>
														<select name="id_sesi" required id="id_sesiwk" readonly disabled  class="form-control">
															<option value="">--- Pilih Jam Pelajaran --</option>
															<?php
															$query = "SELECT * from sesi";
															$hasil = mysql_query($query);
															while ($data = mysql_fetch_array($hasil))
															{
																if ($data['id_sesi']==$row['id_sesi']) {
																	$cek ="selected";
																}
																else{
																	$cek= "";
																}
																echo "<option value=\"$data[id_sesi]\" $cek>$data[jam]</option>\n";
															}
															?>
														</select>
													</div>
										...
										<div id="modaledit<?php echo $id_jadwal ;?>" class="uk-modal">
													...
													$('#guruwk').keyup(function(){  
														...
														{  
															$.ajax({  
																...
																{  
																	$('#guruListwk').fadeIn();  
																	$('#guruListwk').html(data);  
																}  
															});  
														}  
													});  
													$(document).on('click', 'li', function(){  
														$('#guruwk').val($(this).text());  
														$('#guruListwk').fadeOut();  
											...
											<script type="text/javascript">
												...
												function jikukkelasedit(){
													var id_guru = $("#guruwk").val();
													var kd_mapel = $("#kd_mapelwk").val();
													$.ajax({
														url: "inc/jikuk_kelas_mengajar.php",
														data: "kd_mapel="+kd_mapel+"&id_guru="+id_guru,
														cache: false,
												...
												function jikuksesiedit(){
													var id_guru = $("#guruwk").val();
													var kd_mapel = $("#kd_mapelwk").val();
													var id_kelas = $("#id_kelaswk").val();
													var id_hari = $("#id_hariwk").val();
													$.ajax({
														url: "inc/jikuk_sesi_mengajar.php",
														data: "kd_mapel="+kd_mapel+"&id_guru="+id_guru+"&id_kelas="+id_kelas+"&id_hari="+id_hari,
														cache: false,
...
<div class="uk-modal-dialog">
	...
	<form role="form" method="post" action="jadwal.action?act=update&&id_jadwal=<?php echo $id_jadwal;  ?>" enctype="multipart/form-data" >
		<div class="form-group">
			...
			<input type="text" name="guru" required id="guruwk" onkeyup="asdedit()" value="<?php echo ucwords($row["nm_guru"]).' {' .$row["nip"]. ' }' ?>" class="form-control" placeholder="Cari Nama / NIP" />  
			<input type="hidden" name="id_gurus" id="id_gurus"  class="form-control" />  
		...
		<div class="form-group">
			<label>Mata Pelajaran</label>
			<select name="kd_mapel" required id="kd_mapelwk" onchange="jikukkelasedit()" class="form-control">
				<option value="">--- Pilih Mapel --</option>
				<?php
		...
		<div class="item form-group">
			<label>Pilih Kelas<span class="required">*</span></label>
			<div>
				<select name="id_kelas" id="id_kelaswk" required class="form-control">
					<option value=""> Pilih Kelas </option>
					<?php
		...
		<div class="item form-group">
			<label>Pilih Hari<span class="required">*</span></label>
			<div>
				<select name="id_hari" id="id_hariwk" required class="form-control">
					<option value="">--- Pilih Hari <?php echo $row['id_hari']; ?> --</option>
					<?php
					$queryhari = "SELECT * from hari";
					$hasilhari = mysql_query($queryhari);
		...
		<div class="item form-group">
			<label>Pilih Jam Pelajaran<span class="required">*</span></label>
			<div>
				<select name="id_sesi" required id="id_sesiwk" class="form-control">
					<option value="">--- Pilih Jam Pelajaran --</option>
					<?php
					$query = "SELECT * from sesi";
					$hasil = mysql_query($query);
