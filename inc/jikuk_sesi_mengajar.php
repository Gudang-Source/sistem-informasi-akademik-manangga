...
$sesi = mysql_query("SELECT id_sesi, jam FROM kelas where id_kelas in (SELECT kelas.id_kelas FROM kelas left join mengajar on mengajar.id_kelas=kelas.id_kelas 
					where mengajar.kd_mapel='$kd_mapel' and mengajar.id_guru='$id_guru1'
					) order by nm_kelas asc");
echo "<option>-- Pilih Kelas --</option>";
while($k = mysql_fetch_array($sesi)){
