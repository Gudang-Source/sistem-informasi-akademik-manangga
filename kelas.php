						  ...
						  while ($row=mysql_fetch_array($exe)) { $no++; ?>
							  ...
							  <tr>
								...
								<td width="15%"><div class="uk-text-center">
								  <a href="kelas.update?id=<?php echo $row[id_kelas]?>" title="Sunting" data-uk-tooltip="{pos:'top-left'}" class="uk-button uk-button-small"><i class="uk-icon-pencil"></i></a>
								  <a href="kelas.hapus?id=<?php echo $row[id_kelas]?>" onclick="return confirm('Apakah anda yakin akan menghapus data guru: <?php echo $row[1] ?> ini?')" title="Hapus" data-uk-tooltip="{pos:'top-left'}" class="uk-button uk-button-small uk-button-danger"><i class="uk-icon-remove"></i></a></div>
								</td>
								<?php } ?>						
							  </tr>
							  <?php  } ?>
